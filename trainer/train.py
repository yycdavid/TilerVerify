...
from model import CNN_small, CNN_lidar
from dataset import RoadSceneDataset, LidarDataset
...
def train_lidar(args, model, device, train_loader, optimizer, epoch):
    model.train()
...
def test_lidar(args, model, device, test_loader):
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
def load_data_lidar(data_dir, train_file, val_file):
    with open(os.path.join(data_dir, train_file), 'rb') as f:
        train_data = pickle.load(f)
    with open(os.path.join(data_dir, val_file), 'rb') as f:
...
def main_for_lidar():
    ...
    data_dir = os.path.join(base_dir, 'data', args.dataset_folder)
    train_data, eval_data = load_data_lidar(data_dir, args.train_data, args.val_data)

    train_dataset = LidarDataset(train_data['images'], train_data['labels'].squeeze())
    eval_dataset = LidarDataset(eval_data['images'], eval_data['labels'].squeeze())
    train_loader = torch.utils.data.DataLoader(
        train_dataset,
        batch_size=args.batch_size, shuffle=True)
    eval_loader = torch.utils.data.DataLoader(
    ...
    for epoch in range(1, args.epochs + 1):
        if (epoch - best_epoch) > MAX_NON_IMPROVING_EPOCHS:
            break
        train_lidar(args, model, device, train_loader, optimizer, epoch)
        eval_loss, eval_acc = test_lidar(args, model, device, eval_loader)
        if best_valid_loss is None or eval_loss < best_valid_loss:
            outputManager.say(
                "Found better model ({} < {}) @ epoch {}".format(
                    eval_loss,
