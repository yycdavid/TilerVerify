...
then
    ...
    do
        ...
        do
            ...
            do
                python -m eevbnn eval_bin -e 0.3 --timeout 10 results_tiler/lidar_quant_061_small/last.pth --check-cvt --lidar --shape $shape --thread $thread_number --lidar-data-folder $DATA_FOLDER &
                sleep .5
            done
        done
        wait

        python collect_result_lidar.py --exp_dir $DATA_FOLDER --num_threads $num_threads_per_class

        python heatmap_lidar.py --result_dir data/"$DATA_FOLDER" --distance_min 30 --distance_max 60 --angle_range 45
    done
fi
