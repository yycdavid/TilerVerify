...
class Binarize01Act(nn.Module):
    class Fn(Function):
        ...
        def backward(ctx, g_out):
            ...
            # return g_self.mul_(g_out)

            # grad as sign(tanh(x))
            tanh = torch.tanh(inp)
            return (1 - tanh.mul_(tanh)).mul_(g_out)
...
class BinarizeSignAct(nn.Module):
    class Fn(Function):
        @staticmethod
        def forward(ctx, inp):
            ctx.save_for_backward(inp)
        ...
        @staticmethod
        def backward(ctx, g_out):
            return Binarize01Act.Fn.backward(ctx, g_out)
...
class Binarize01WeightScaleFn(Function):
    @staticmethod
    def forward(ctx, inp):
        v_sum = v.sum()
        scale = F.relu(inp).sum() / v.sum()
        ctx.save_for_backward(v, v_sum, scale)
        return v

    @staticmethod
    def backward(ctx, g_out):
        v, v_sum, scale = ctx.saved_tensors
        ret = (v / v_sum).add_(scale).mul_(g_out).clamp_(-1, 1)
        return ret
...
def binarize_weights(layer: nn.Module):
    ...
    if wb is TernaryWeightFn:
        return wb.apply(weight)
    if wb is TernaryWeightWithMaskFn:
        mask = layer._parameters.get('weight_mask')
        if mask is None:
            layer.register_parameter(
                'weight_mask', nn.Parameter(
                    torch.empty_like(weight).normal_(std=0.01).abs_().detach_()
                ))
            mask = layer._parameters['weight_mask']
        return wb.apply(weight) * Binarize01WeightNoScaleFn.apply(mask)

    assert wb in [Binarize01WeightNoScaleFn, Binarize01WeightScaleFn]
    mask = layer._buffers.get('weight_mask')
    if mask is None:
        bias = 0.6  # XXX: need tuning and more explanation
        u = (torch.empty_like(weight).
             uniform_(-1 + bias, 1 + bias).
             sign_().
             detach_())
        layer.register_buffer('weight_mask', u)
        mask = layer._buffers['weight_mask']
    return wb.apply(weight) * mask
    ...
    def forward(self, inp: torch.Tensor):
        ...
        if self.training:
            ...
            with torch.no_grad():
                self.running_var = (
                    self.running_var.
                    mul_(1 - self.momentum).
                    add_(var * self.momentum).
                    detach_()
                )
                self.running_mean = (
                    self.running_mean.
                    mul_(1 - self.momentum).
                    add_(mean * self.momentum).
                    detach_()
                )
        else:
...
class RoadSign0(SeqBinModelHelper, nn.Module, ModelHelper):
    ...
    def _setup_network(self, quant_step):
        self.make_small_network(self, quant_step, 1, 8)
    ...
    def make_small_network(
        ...
        self.features = nn.Sequential(
            ...
            lin(wb, chl[2], 3),
            setattr_inplace(
                BatchNormStatsCallbak(self, 3, use_scalar_scale=True),
                'bias_regularizer_coeff', 0)
        )

    @classmethod
    def make_dataset_loader(cls, args, train: bool):
        dataset_folder = os.path.join(args.data, 'lidar_train_small_gaussian_0.001')

        if train:
            loaded_data = load_data_lidar(dataset_folder, 'train.pickle')
        else:
            loaded_data = load_data_lidar(dataset_folder, 'valid.pickle')

        dataset = LidarDataset(loaded_data['images'], loaded_data['labels'].squeeze())
...
def load_data_lidar(data_dir, data_file):
