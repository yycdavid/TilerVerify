...
def encode_input_lidar(env, quant_step: float, image_lower_bound: np.ndarray, image_upper_bound: np.ndarray, *, range_min=0, range_max=1):
    """encode for per-dim L-inf bounded input perturbations"""
    assert 0 < quant_step < 1
    inp_min = np.clip(image_lower_bound, range_min, range_max)
    inp_max = np.clip(image_upper_bound, range_min, range_max)
    inp_sym = QuantizedInputRangeTensor.from_value_range(
        env, inp_min, inp_max, quant_step, 'inp'
    )
    return inp_sym
...
class ModelVerifierLidar:
    ...
    def check_and_save(self, data_idx: int, image_lower_bound: np.ndarray, image_upper_bound: np.ndarray, distance_lower_bound: float, distance_upper_bound: float, angle_lower_bound: float, angle_upper_bound: float, label: np.ndarray):
        """run :meth:`check` and write the result to given result file"""
        check_result, (time_b, time), _ = self.check(image_lower_bound, image_upper_bound, label)

        self._results[data_idx] = {
            'result': str(check_result),
            'build_time': time_b,
            'solve_time': time,
            'distance_min': distance_lower_bound,
            'distance_max': distance_upper_bound,
            'angle_min': angle_lower_bound,
            'angle_max': angle_upper_bound,
        }
    ...
    def check(self, image_lower_bound: np.ndarray, image_upper_bound: np.ndarray, label: np.ndarray):
        """verify model on a single input without writing to result file
        ...
        :return: (check_result, (time_build, time_verify), inp_adv)
        """
        assert image_lower_bound.ndim == 3 and label.size == 1
        env, check_result, (time_b, time), clsfy_scores, inp_adv = self._work(
            image_lower_bound[np.newaxis], image_upper_bound[np.newaxis], label.reshape((1, )))
        ...
        if check_result == SolveResult.UNSAT:
            ...
            inp_adv, = inp_adv # remove the batch dim
            #check_score([i for (i,) in clsfy_scores], inp_adv)
        else:
            assert check_result == SolveResult.TLE
    ...
    def _build_sat_env(self, env, image_lower_bound, image_upper_bound, label):
        if self._cached_clause_info is not None:
            return self._build_sat_env_with_cache(env, image_lower_bound, image_upper_bound, label)
        if not self.args.disable_model_cache:
            if env.make_clause_recorder() is None:
                self.args.disable_model_cache = True
            else:
        ...
        inp_sym = encode_input_lidar(env, self.models[0].quant_step,
                                    image_lower_bound, image_upper_bound)
        return self._build_sat_env_combine_model(
            inp_sym, label,
            (encode_model(env, i, inp_sym) for i in self.models))
    ...
    def _build_sat_env_with_cache(self, env, image_lower_bound, image_upper_bound, label):
        first_layer_type = (BinConv2dEval, BinLinearEval)
        if self._cached_clause_info is None:
            need_replay = False
            assert (self.args.var_preference in
    ...
    def _work(self, image_lower_bound, image_upper_bound, label):
        assert image_lower_bound.min() >= 0 and image_upper_bound.max() <= 1
        ...
        t0 = time.time()
        inp_sym, all_ftr_syms, all_clsfy_layers = self._build_sat_env(
            env, image_lower_bound, image_upper_bound, label)
        if args.write_formula:
            with open(args.write_formula, 'w') as fout:
                env.write_formula(fout)
        build_time = time.time() - t0
...
def init_argparser(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--sat-solver', choices=['z3', 'pysat', 'pysat_stat'],
...
def main():
    ...
    parser.add_argument('--lidar', action='store_true', help='run for lidar experiment')
...
def main_lidar(args):
    ...
    master_model, _ = load_model_and_cvt(args, device, args.model)
    models = [master_model]

    verifier = ModelVerifierLidar(args, models)
    ...
    indices = verify_data['index'][0]
    image_lower_bounds = verify_data['image_lower_bounds'] # (N, H, W)
    image_upper_bounds = verify_data['image_upper_bounds']
    distance_lower_bounds = verify_data['distance_lower_bounds'][0] # (N,)
    distance_upper_bounds = verify_data['distance_upper_bounds'][0]
    angle_lower_bounds = verify_data['angle_lower_bounds'][0]
    angle_upper_bounds = verify_data['angle_upper_bounds'][0]
    label_as_arr = np.array([label])

    for i in range(len(indices)):
        verifier.check_and_save(
            indices[i].item(),
            image_lower_bounds[i:i+1],
            image_upper_bounds[i:i+1],
            distance_lower_bounds[i],
            distance_upper_bounds[i],
            angle_lower_bounds[i],
            angle_upper_bounds[i],
            label_as_arr,
        )
