...
class SatEnv(metaclass=ABCMeta):
    ...
    @abstractmethod
    def get(self, v) -> int:
        """get the value of a var after :meth:`solve` returns ``True``"""
...
def gen_le(env: SatEnv, v0: np.ndarray, v1: np.ndarray):
    """generate an expression for ``v0 <= v1`` under the bit interpretation,
    where ``v0`` are :class:`BoolRef` objects and ``v1`` is a constant"""
