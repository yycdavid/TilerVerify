...
class PySatEnv(SatEnv):
    ...
    def __init__(self, name, need_record):
        ...
        self._card_encoder = CardEncoder(self)
    ...
    def _do_new_var(self, name, varid):
        ...
        return PySatBoolRef(self, var=varid)
    ...
    def make_at_least(self, props, bound: int, name: str):
        ...
        return PySatBoolRef(self, var=ret)
...
class PySatBoolRef(BoolRef):
    ...
    __slots__ = ['env', 'expr', 'var']

    def __init__(self, env: PySatEnv, *, expr=None, var=None):
        assert int(expr is None) + int(var is None) == 1
        assert type(env) is PySatEnv or type(env) is PySatStatEnv
        self.env = env
        if expr is not None:
    ...
    def __neg__(self):
        ...
        return PySatBoolRef(self.env, var=-self.var)
    ...
    def __and__(self, rhs):
        ...
        return PySatBoolRef(self.env, expr=self.clauses + rhs.clauses)
    ...
    def __or__(self, rhs):
        ...
        if self.var is not None:
            ...
            c = self.clauses
            v = rhs.var
        return PySatBoolRef(self.env, expr=[i + [v] for i in c])
