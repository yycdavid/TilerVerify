...
noise_mode=uniform
noise_scale=0.01
#python3 generate_data.py --mode train --offset_range $OFFSET_RANGE_TRAIN --angle_range $ANGLE_RANGE_TRAIN --noise $noise_mode --noise_scale $noise_scale --target_dir_name train_"$noise_mode"_"$noise_scale"
...
do
    ...
    data_name=verify_offset_"$OFFSET_RANGE"_angle_"$ANGLE_RANGE"_grid_"$grid_size"_thread_"$num_threads""$noise_mode""$noise_scale"
